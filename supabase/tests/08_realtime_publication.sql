...
-- order_messages (0037) / delivery_photos (0039) — both community-scoped
-- and both want live updates (a chat thread; a photo appearing on an
-- open order detail), so unlike order_events they ARE published.
select ok(
...
select ok(
  exists (select 1 from pg_publication_tables where pubname = 'supabase_realtime' and schemaname = 'public' and tablename = 'delivery_photos'),
  'item 13c: delivery_photos IS a member of supabase_realtime'
);

-- ================================================================
-- item 14: exactly twelve tables total — catches a member added under some
...
select is(
  ...
  12,
  'item 14: supabase_realtime has exactly the twelve approved members, nothing else'
);
