...
function describeItemsChange(from, to) {
  ...
  for (const [k, it] of toMap) {
    if (!fromMap.has(k)) bits.push(`added ${name(it)}`);
      ...
      if (before !== after) bits.push(`${name(it)} ${before} to ${after}`);
    }
  }
  ...
  if (bits.length === 0) return fromArr.length === toArr.length ? 'items updated' : `items ${fromArr.length} to ${toArr.length}`;
  if (bits.length > 3) return `items ${fromArr.length} to ${toArr.length}`;
  return bits.join(', ');
}
...
export const EVENT_RENDER = {
  ...
  approval_reverted: (e, label) => ({ icon: '', text: `${e.actorName || 'Owner'} reverted the decision on ${label}` }),
  purchase_started: (e, label) => ({ icon: '', text: `${e.actorName || 'A buyer'} started purchasing ${label}` }),
  purchase_abandoned: (e, label) => ({ icon: '', text: `${e.actorName || 'A buyer'} released ${label} back to the purchase queue` }),
  ...
  delivery_cancelled: (e, label) => ({ icon: '', text: `${e.actorName || 'A driver'} cancelled ${label}${e.reason ? `: ${e.reason}` : ''}` }),
  ...
  order_edited: (e, label) => {
    ...
    const parts = Object.entries(changes)
      ...
      .map(([field, { from, to }]) => `${fieldLabels[field] || field} ${escapeHtml(from ?? '(blank)')} to ${escapeHtml(to ?? '(blank)')}`);
    ...
    // otherwise read "needed by deadline to deadline". Say something honest
    ...
    if (changes.neededByType) {
      ...
      parts.push(from === to
        ...
        : `needed by ${NEEDED_BY_TYPE_WORDS[from ?? 'null']} to ${NEEDED_BY_TYPE_WORDS[to ?? 'null']}`);
    }
    ...
    return { icon: '', text: `${e.actorName || 'The worker'} edited ${label}${parts.length ? `: ${parts.join(', ')}` : ''}` };
  },
  ...
  cancellation_rejected: (e, label) => {
    if (e.meta?.autoClosed) {
      return { icon: '', text: `The cancellation request for ${label} could no longer be decided${e.reason ? `: ${e.reason}` : ''}` };
    }
